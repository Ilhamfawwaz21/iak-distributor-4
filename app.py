...
def get_order_data():
    docs = orders_ref.stream()  # Assuming orders_ref is a reference to the data

    orders = []
    for doc in docs:
        # Convert Firestore document snapshot to dictionary
        doc_data = doc.to_dict()  # Assuming doc is a Firestore document snapshot
        
        order_data = {
            'id_log': doc_data.get('id_log', ''),         # Default to empty string if not found
            'berat': doc_data.get('berat', 0),            # Default to 0 for weight if not found
            'status': doc_data.get('status', ''),         
            'kota_asal': doc_data.get('kota_asal', ''),
            'kota_tujuan': doc_data.get('kota_tujuan', ''),
            'resi': doc_data.get('resi', '')              
        }
...
def add_order():
    ...
    # Validasi input
    if not all([id_log, kota_asal, kota_tujuan]):
...
def update_order(resi):
    ...
    # Validasi input
    if not all([id_log, resi, status, kota_asal, kota_tujuan, berat]):
        return redirect(url_for('home_order', message='Semua field harus diisi!', success=False))
